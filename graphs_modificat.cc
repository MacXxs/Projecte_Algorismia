...
UndirectedGraph vertex_percolation(const UndirectedGraph& g, const double p){
   UndirectedGraph g_per = UndirectedGraph(g);
   ...
   for(next = vi; next != vi_end; vi = next) {
       ...
       ++next;
       if (p1 <= p) {
           boost::clear_vertex(*vi, g_per);
           /*boost::tie(ai, ai_end) = boost::adjacent_vertices(*vi,g_per);
           for (adjacency_iterator it = ai; it != ai_end; ai = it){
               ++it;
               if (*ai > *vi) {
                   cout << "Elimina l'aresta: (" << *vi << ',' << *ai << ')' << endl;
                   boost::remove_edge(*vi, *ai, g_per);
               }
           }*/
           boost::remove_vertex(*vi, g_per);
       }
   }
   return g_per;
}
...
UndirectedGraph vertex_percolation(const UndirectedGraph& g, const double p){
    ...
    for (int i = 0; i < vertices.size(); ++i){
        ...
        if (p1 <= p) {
            ...
            //boost::clear_in_edges(i, g_per);
            //boost::clear_out_edges(i, g_per);
